...
// `expo-camera`'s `CameraView` isn't native-only: on web it drives the
// browser's `getUserMedia` feed and decodes frames itself (see
// `expo-camera`'s `ExpoCamera.web.js`), so the same component and the same
// `onBarcodeScanned` callback cover a phone's camera and a laptop's webcam
    ...
    <Screen edges={["top", "bottom"]} statusBarStyle="light" fullBleed className="bg-black">
      <View className="flex-1">
        {permission?.granted ? (
          ...
          <PermissionPrompt permission={permission} onRequest={requestPermission} />
        )}
  ...
  return (
    <View className="flex-1 items-center justify-center gap-4 px-8">
      ...
      <Button
        ...
        className="mt-2"
      >
        {canAskAgain ? "Allow camera access" : Platform.OS === "web" ? "Enable in browser settings" : "Open settings"}
      </Button>
    </View>
  );
}
